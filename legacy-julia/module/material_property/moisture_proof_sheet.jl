...
### 水分特性 ###
# 含水率
get_phi( ;rh::Float64 ) = rh * 0.001
get_phi( cell ) = get_phi( rh = convertMiu2RH( temp = cell.temp, miu = cell.miu ) )

# 含水率の水分化学ポテンシャル微分
# dphi/dmiu = dphi/drh * drh/dmiu
get_dphi( ;temp::Float64, miu::Float64 ) = 0.001 * ( 1.0 / Rv / temp ) * exp( miu / Rv / temp )
...
const zp = 1.88e6 # 透湿抵抗[m2･s･kg/kgDA]/kg
