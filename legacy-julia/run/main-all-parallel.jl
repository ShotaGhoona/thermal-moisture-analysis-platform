#=
================================================================================
建物内温湿度環境解析ネットワークモデル - 全パターン並列実行
================================================================================

使い方:
    julia -t 4 run/main-all-parallel.jl    # 4スレッドで並列実行
    julia -t auto run/main-all-parallel.jl  # 利用可能な全コアを使用

機能:
    - 5(wall) × 5(opening) × 3(climate) = 75パターンを並列実行
    - マルチスレッドによる高速化
    - スレッドごとのログファイル（競合回避）
    - 進捗管理ファイルで状況確認可能
    - エラー発生時も続行し、後で確認可能
...
出力先:
    ...
    │   ├── _case_log.txt      # ケースごとのログ
    │   └── ...
    └── ...

注意:
    - スレッド数は julia -t N で指定（デフォルト: 1）
    - 推奨スレッド数: 物理コア数以下
    - メモリ使用量はスレッド数に比例して増加
=#
...
const BATCH_DIR_NAME = "batch_all_parallel"
...
println("🚀  全パターン並列実行 (Parallel Batch Runner)")
println("🚀 ════════════════════════════════════════════════════════════════")
println("🚀  起動時刻: ", now())
...
if nthreads() == 1
    println("⚠️  警告: シングルスレッドで実行中")
    println("   並列実行には julia -t N で起動してください")
    println("   例: julia -t 4 run/main-all-parallel.jl")
    println()
end
...
const COMPLETED_LOCK = ReentrantLock()
const SUMMARY_LOCK = ReentrantLock()

# アトミックカウンター
const ATOMIC_COMPLETED = Atomic{Int}(0)
const ATOMIC_FAILED = Atomic{Int}(0)
const ATOMIC_RUNNING = Atomic{Int}(0)

# スレッドごとのログファイル
const THREAD_LOGS = Dict{Int, IOStream}()
const THREAD_LOGS_LOCK = ReentrantLock()
...
"""スレッドログの初期化"""
function setup_thread_logs(batch_dir::String)
    thread_log_dir = joinpath(batch_dir, "_thread_logs")
    mkpath(thread_log_dir)

    for tid in 1:nthreads()
        log_path = joinpath(thread_log_dir, "thread_$(tid).log")
        THREAD_LOGS[tid] = open(log_path, "w")
        println(THREAD_LOGS[tid], "═" ^ 70)
        println(THREAD_LOGS[tid], "Thread $tid Log - Started at $(now())")
        println(THREAD_LOGS[tid], "═" ^ 70)
        println(THREAD_LOGS[tid])
        flush(THREAD_LOGS[tid])
    end
end

"""スレッドログのクローズ"""
function close_thread_logs()
    lock(THREAD_LOGS_LOCK) do
        for (tid, f) in THREAD_LOGS
            println(f)
            println(f, "═" ^ 70)
            println(f, "Thread $tid Log - Ended at $(now())")
            println(f, "═" ^ 70)
            close(f)
        end
        empty!(THREAD_LOGS)
...
"""スレッドログに書き込み"""
function thread_log(msg::String)
    tid = threadid()
    lock(THREAD_LOGS_LOCK) do
        if haskey(THREAD_LOGS, tid)
            timestamp = Dates.format(now(), "HH:MM:SS")
            println(THREAD_LOGS[tid], "[$timestamp] $msg")
            flush(THREAD_LOGS[tid])
        end
    end
end
...
"""ケースログに書き込み（ケースフォルダ内）"""
function case_log(case_output_dir::String, msg::String)
    log_file = joinpath(case_output_dir, "_case_log.txt")
    timestamp = Dates.format(now(), "yyyy-mm-dd HH:MM:SS")
    open(log_file, "a") do f
        println(f, "[$timestamp] $msg")
    end
end
...
"""進捗ファイルを更新（スレッドセーフ）"""
function update_progress(batch_dir::String, total::Int;
                         start_time::DateTime=now(),
                         running_cases::Vector{String}=String[])

    completed = ATOMIC_COMPLETED[]
    failed = ATOMIC_FAILED[]
    running = ATOMIC_RUNNING[]
    ...
    lock(PROGRESS_LOCK) do
        ...
        # 進捗率
        progress_pct = round(completed / total * 100, digits=1)

        # プログレスバー
        bar_width = 50
        filled = Int(floor(bar_width * completed / total))
        ...
        open(progress_file, "w") do f
            println(f, "")
            println(f, "🚀 ════════════════════════════════════════════════════════════════")
            println(f, "🚀  建物内温湿度環境解析 - 並列バッチ実行進捗")
            println(f, "🚀  スレッド数: $(nthreads())")
            println(f, "🚀 ════════════════════════════════════════════════════════════════")
            println(f, "")
            println(f, "   [$bar] $progress_pct%")
            println(f, "")
            println(f, "   📊 進捗状況: $completed / $total パターン")
            println(f, "")
            println(f, "   ┌──────────────────────────────────────────────────")
            println(f, "   │  ✅ 完了:     $completed")
            println(f, "   │  ❌ 失敗:     $failed")
            println(f, "   │  🔄 実行中:   $running")
            println(f, "   │  ⏳ 残り:     $(total - completed - running)")
            println(f, "   └──────────────────────────────────────────────────")
            println(f, "")
            if !isempty(running_cases)
                println(f, "   🔄 実行中のケース:")
                for (i, case_id) in enumerate(running_cases)
                    prefix = i == length(running_cases) ? "└─" : "├─"
                    println(f, "      $prefix $case_id")
                end
                println(f, "")
            end
            println(f, "   ⏱️  経過時間: $elapsed_str")
            println(f, "   ⏰ 残り時間: $remaining_str")
            println(f, "")
...
function write_summary_header(batch_dir::String)
    ...
    if !isfile(summary_file)
        open(summary_file, "w") do f
            println(f, "case_id,wall_id,wall_name,opening_id,opening_name,climate_id,climate_name,status,duration_sec,start_time,end_time,thread_id,error_message")
        end
    end
end

"""サマリCSVに結果を追記（スレッドセーフ）"""
function append_summary(batch_dir::String, case_id::String,
    ...
    lock(SUMMARY_LOCK) do
        ...
        open(summary_file, "a") do f
            println(f, "$case_id,$(wall.id),$(wall.name),$(opening.id),$(opening.name),$(climate.id),$(climate.name),$status,$duration_sec,$(Dates.format(start_time, "yyyy-mm-dd HH:MM:SS")),$(Dates.format(end_time, "yyyy-mm-dd HH:MM:SS")),$thread_id,\"$error_escaped\"")
        end
    end
end
...
"""単一パターンのシミュレーションを実行（スレッドセーフ版）"""
function run_single_simulation(wall, opening, climate, output_dir::String, case_id::String)
    tid = threadid()

    thread_log("開始: $case_id")
    case_log(output_dir, "シミュレーション開始")
    case_log(output_dir, "スレッドID: $tid")
    ...
    case_log(output_dir, "入力ファイル:")
    case_log(output_dir, "  Room: $input_room")
    case_log(output_dir, "  Wall: $input_wall")
    case_log(output_dir, "  Opening: $input_opening")
    case_log(output_dir, "  Climate: $input_climate")
    ...
    case_log(output_dir, "BNMモデル作成完了")
    case_log(output_dir, "  Rooms: $(length(network_model.rooms)) 室")
    case_log(output_dir, "  Walls: $(length(network_model.walls)) 壁")
    case_log(output_dir, "  Openings: $(length(network_model.openings)) 開口")
    ...
    case_log(output_dir, "計算ループ終了")
    case_log(output_dir, "総ステップ数: $step_count")
    ...
    case_log(output_dir, "完了: $(format_duration(calc_elapsed))")
    thread_log("完了: $case_id ($(format_duration(calc_elapsed)))")
...
function main()
    ...
    try
        ...
        initial_completed = length(completed_set)
        ATOMIC_COMPLETED[] = initial_completed

        if initial_completed > 0
            println("🔄 再開モード検出:")
            println("   ├─ 完了済み: $initial_completed パターン")
            println("   └─ 残り: $(total - initial_completed) パターン")
            println()
        end
        ...
        log_message(batch_dir, "🚀 並列バッチ実行開始", also_print=false)
        log_message(batch_dir, "   総パターン数: $total", also_print=false)
        log_message(batch_dir, "   スレッド数: $(nthreads())", also_print=false)
        log_message(batch_dir, "   スキップ済み: $initial_completed", also_print=false)
        log_message(batch_dir, "════════════════════════════════════════════════════════", also_print=false)
        ...
        # 実行中のケースを追跡
        running_cases = Vector{String}()
        running_cases_lock = ReentrantLock()

        println()
        println("🔄 ════════════════════════════════════════════════════════════════")
        println("🔄  並列バッチ実行開始 ($(nthreads())スレッド)")
        println("🔄 ════════════════════════════════════════════════════════════════")
        println()

        # 進捗更新タスク
        progress_task = @async begin
            while ATOMIC_COMPLETED[] + ATOMIC_FAILED[] < total - initial_completed + initial_completed
                lock(running_cases_lock) do
                    update_progress(batch_dir, total,
                                   start_time=batch_start_time,
                                   running_cases=copy(running_cases))
                end
                sleep(5)  # 5秒ごとに更新
            end
        end
        ...
        @threads for case in all_cases
            ...
            # 実行中リストに追加
            lock(running_cases_lock) do
                push!(running_cases, case_id)
            end
            atomic_add!(ATOMIC_RUNNING, 1)

            # 出力ディレクトリ作成
            case_output_dir = joinpath(batch_dir, case_id)
            mkpath(case_output_dir)

            log_message(batch_dir, "開始: $case_id", also_print=true)

            case_start_time = now()

            try
                duration = run_single_simulation(case.wall, case.opening, case.climate, case_output_dir, case_id)

                case_end_time = now()

                atomic_add!(ATOMIC_COMPLETED, 1)
                mark_completed(batch_dir, case_id)

                append_summary(batch_dir, case_id, case.wall, case.opening, case.climate,
                              "SUCCESS", duration, case_start_time, case_end_time, tid)

                log_message(batch_dir, "完了: $case_id ($(format_duration(duration)))", also_print=true)

            catch e
                case_end_time = now()
                duration = Dates.value(case_end_time - case_start_time) / 1000

                atomic_add!(ATOMIC_FAILED, 1)
                error_msg = sprint(showerror, e)

                append_summary(batch_dir, case_id, case.wall, case.opening, case.climate,
                              "FAILED", duration, case_start_time, case_end_time, tid, error_msg)
                ...
                # 実行中リストから削除
                lock(running_cases_lock) do
                    filter!(x -> x != case_id, running_cases)
                end
                atomic_sub!(ATOMIC_RUNNING, 1)
            end
        end
        ...
        completed_count = ATOMIC_COMPLETED[]
        failed_count = ATOMIC_FAILED[]
        ...
        # 最終ログ
        log_message(batch_dir, "════════════════════════════════════════════════════════", also_print=false)
        log_message(batch_dir, "🎉 並列バッチ実行完了", also_print=false)
        ...
        println("      ├─ ⏭️  スキップ:    $initial_completed")
        println("      ├─ 🖥️  スレッド数:  $(nthreads())")
        ...
        println("      ├─ _thread_logs/     スレッドごとのログ")
        ...
        # 速度向上の表示
        if nthreads() > 1 && completed_count > initial_completed
            avg_time = total_elapsed / (completed_count - initial_completed)
            serial_estimated = avg_time * total
            speedup = serial_estimated / total_elapsed
            println("   ⚡ パフォーマンス:")
            println("      ├─ 1パターンあたり平均: $(format_duration(avg_time))")
            println("      └─ 推定速度向上: $(round(speedup, digits=2))x")
            println()
        end
