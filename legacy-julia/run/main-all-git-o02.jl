#=
================================================================================
建物内温湿度環境解析ネットワークモデル - 全パターン一括実行 (Git自動Push版)
================================================================================

使い方:
    julia run/main-all-git.jl
...
注意:
    - 事前にGitHub認証設定が必要（SSHキーまたはパーソナルアクセストークン）
    - git push失敗時も計算は継続

出力先:
    output_data/batch_all_o02/{RUN_ID}/
...
const BATCH_DIR_NAME = "batch_all_o02"
const RUN_ID = "run02"  # 実行ID（任意の文字列、再開時は同じ値を使用）
...
println("🚀  全パターン一括実行 (Batch Runner) + Git自動Push版")
println("🚀 ════════════════════════════════════════════════════════════════")
println("🚀  起動時刻: ", now())
